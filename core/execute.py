import sys, random, time, math, datetime, signal, calendar, logging, subprocess, os
...
def get_internal_pid():
	pass

def spawn_vm(seq_num, vm_chars, wait_until_finished, node, port):
	'''
	...
	udata = get_vm_userdata(seq_num, vcpus, bench, runtime, times_to_run, port)
...
#	            getattr(new_vm, 'OS-EXT-SRV-ATTR:host'), vcpus)
	...
	return (new_vm_uuid, new_vm)

def count_and_delete_finished_vms(node):
	err = 0
	shutoff = 0
	deleted = []
	for vm in ost_client.get_vms():
		if not vm.name.startswith(node + "-"):
			continue
		if vm.status == "ERROR":
			err += 1
...
def wait_for_vms_to_finish(timeout_minutes):
	...
	while 1:
		...
		if (time.time() - wait_vms_start_time > timeout_minutes * 60.0):
			...
			stucked_vms = ost_client.delete_existing_vms(prefix="acticloud4-")
...
def generate_vms_queue_middleware():
	'''
	This is used to generate the scenario that demonstrates ActiManager.internal for
	the presentation of Middleware
	'''
	global ISGOLD_PROPS 
	global RUNTIMES
	...
	ISGOLD_PROPS[0][0] = 100
	ISGOLD_PROPS[0][1] = 0
	vms_array = [ (8, 1, 1, 1), (4, 1, 1, 1) ] ## These are the two A-class citizens :-D
	nvcpus = 20 ## How many additional vcpus to add?

	## For the External demo
#	vms_array = [ (4,1,1,1),
#	              (4,1,0,0),
#	              (4,1,1,1),
#	              (2,1,0,0),
#	              (2,1,0,0) ]
#	nvcpus = 10 ## How many additional vcpus to add?

	vms = []

	## First add the two A-class citizens
	for v in vms_array:
		if (len(v) == 0):
			## Silver VMs
			vm_chars = get_new_vm_characteristics()
#			vm_chars['bench'] = benches[-1] if vm_chars['is_noisy'] else benches[-13] ## omnetpp or gamess
		else:
			## Gold VMs
			vm_chars = dict()
			vm_chars['nr_vcpus'] = v[0]
			vm_chars['is_gold'] = v[1]
			vm_chars['is_noisy'] = v[2]
			vm_chars['is_sensitive'] = v[3]
			vm_chars['bench'] = benches[2] if v[3] else benches[11] ## astar or gobmk
			vm_chars['runtime'] = random.choice(RUNTIMES)
		vms.append([0.0, vm_chars])

	## The first batch, to reach the desired starting utilization
	while nvcpus > 0:
		new_vm = get_new_vm_characteristics()
#		new_vm['bench'] = benches[-1] if new_vm['is_noisy'] else benches[-13]
...
def generate_vms_queue(nvms = 200, nvcpus_start = 40, rate = 2.0):
	...
	while nvcpus > 0:
		new_vm = get_new_vm_characteristics()
		if new_vm['is_gold']:
			new_vm['bench'] = benches[0] ## Gold VMs are 2-core astar
			new_vm['nr_vcpus'] = 2
			new_vm['is_sensitive'] = 1
			new_vm['is_noisy'] = 0
		vms.append([0.0, new_vm])
		nvcpus -= new_vm['nr_vcpus']
		nvms -= 1
...
#	vms = generate_vms_queue(nvms=25, nvcpus_start = nvcpus_start, rate = rate)
	vms = generate_vms_queue_middleware()
	print_vms_queue(vms)
