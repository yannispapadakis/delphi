...
def prediction(args):
	(func, feature, qos, class_num, model, train) = args[:6]
	...
	if type(model) == str:
		if train == 's': trained_with = 'spec/'
			...
			print("Models have not been trained with " + train + " yet")
			return
	...
	if func == 'cv':
		exclude = set(measures.keys()).difference(get_train_test(train))
	...
	if type(args[4]) == str:
		if func == 'test': print_pred(answers, results_dir + 'predictions/' + args[4] + '/' + '_'.join([args[4], feature, str(class_num), str(qos), func]) + '.csv', np.mean(acc_scores))
		print(func.upper() + ": " + feature + ' | ' + str(qos) + ' | C:' + str(class_num) + ' | ' + args[4] + ' ' * (4 - len(args[4]))+ ' | ' + str(100 * round(np.mean(acc_scores), 4)) + '%')
